...
public:
            ...
            uint16_t SPI_CPHA_x = SPI_CPHA_2Edge)
    {
        ...
        //使能 SPI 的 SCK 信号管脚
        Sm_GPIO_SCK::init_IO(GPIO_Pin_SCK, GPIO_Mode_AF);

        //使能 SPI 的 MOSI 和 MISO 管脚
        switch(SPI_Direction_x)
        {
            case SPI_Direction_1Line_Tx:
                Sm_GPIO_MOSI::init_IO(GPIO_Pin_MOSI, GPIO_Mode_AF); break;
            case SPI_Direction_1Line_Rx:
                Sm_GPIO_MISO::init_IO(GPIO_Pin_MISO, GPIO_Mode_AF); break;
            default:
                Sm_GPIO_MOSI::init_IO(GPIO_Pin_MOSI, GPIO_Mode_AF);
                Sm_GPIO_MISO::init_IO(GPIO_Pin_MISO, GPIO_Mode_AF);
        }
    ...
    static void speedConfig(uint8_t SPI_BaudRatePrescaler_x)
    {
    ...
    {
        Sm_assert(IS_SPI_DATA_SIZE(SPI_DataSize_x));
